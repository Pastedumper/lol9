...
function JSRewriter(ctx) {
  return function JS(data, url) {
    ...
    return data.toString().replace(/(,| |=|\()document.location(,| |=|\)|\.)/gi, global.document = new JSDOM('<script>alert("sug")</script>').window.document, str => { return str.replace('.location', `.$Rhodium.location`); }).replace(/(,| |=|\()window.location(,| |=|\)|\.)/gi, str => { return str.replace('.location', `.$Rhodium.location`); })
    /*return `
(function(window) {
