            ...
            apply: (target, that, args) => {
              ...
              switch(config.action) {
                case 'url':
                  ...
                  if (val&&val.includes('hcaptcha')) val = val.replace('#id=', '?id=')
                  if (!val) val = ''
                  break;
                case 'window':
                  try {
                    if (!val.$Rhodium)  {
                      val = new $Rhodium.Window(val, that.src)
                    } else {
                      val = $Rhodium.Location(new URL(that.src), val)
                      if (that.src.includes('hcaptcha')) val['id'] = new URLSearchParams(that.contentWindow.$Rhodium.location.search).get('id')
                    };
                  } catch(e) {};
                  break;
                case 'srcset':
...
window.Worker = new Proxy(window.Worker, {
  construct(t, a) {
    a[0] = $Rhodium.url.encode(a[0], {Url:new URL($RConfig.url)}).replace('https://', 'https:/')
    console.log(a[0])
...
for (var i = 0; i < LocalStorage.length; i++){
if (LocalStorage.key(i).startsWith($Rhodium.location.host)) length++ 
}; return length
      }*/
      if (name=='key') return LocalStorage.key
        /*function(key) {
        var spoof = []
        Object.keys(LocalStorage).forEach(e => {if (e.startsWith($Rhodium.location.hostname)) spoof.push([e.split($Rhodium.location.hostname)[1], LocalStorage[e]])})
...
$Rhodium.Window = class {
  constructor(window, src) {
    ...
    Spoof.$Rhodium.Location(src, Spoof)
